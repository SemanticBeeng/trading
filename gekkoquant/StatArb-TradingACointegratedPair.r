...
simulateTrading <- function(stockPair) {
  ...
  #Moving average
  movingAvg = rollmean(spread, lookback, na.pad=TRUE) 

  #Moving standard deviation / bollinger bands
  movingStd = rollapply(spread, lookback, sd ,align="right", na.pad=TRUE) 
...
#Daily returns
indexRet <- Delt(Cl(eval(parse(text=paste("symbolData$\"",symbolLst[1],"\"",sep="")))) ,k=1 ,type="arithmetic") 
indexRet <- as.zoo(indexRet)
tradingRetZoo <- indexRet
tradingRetZoo[,1] <- tradingRet
#Convert to zoo objectcolnames(zooTradeVec) <- c("Shell A & B Stat Arb","Shell A")
zooTradeVec <- as.zoo(cbind(tradingRetZoo,indexRet)) 
